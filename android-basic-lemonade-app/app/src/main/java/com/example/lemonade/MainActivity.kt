...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        ...
        var currentState = lemonadeState
        ...
        // TODO: bu kısımlar ayrı ayrı if yapılmış hallerini gösterir
        /*if(currentState == SELECT) {
            lemonSize = lemonTree.pick()
            currentState = SQUEEZE
            squeezeCount = 0
        }
        ...
        //TODO: bu kısım when yapısı ile if statementların kısaltılmasını gösterir
        when(currentState){
            SELECT -> {
            ...
            SQUEEZE -> {
                ...
                this.lemonSize--
                when(this.lemonSize){
                    0 -> currentState = DRINK
                }
            }
            DRINK -> currentState = RESTART
            else -> currentState = SELECT
        }
        lemonadeState = currentState
        setViewElements()
    }
    ...
    private fun setViewElements() {
        ...
        textAction.text = when(lemonadeState){
            ...
            else -> getString(R.string.lemon_empty_glass) // else durumuna restart kalıyor
        }
        ...
        val drawableResource = when(lemonadeState){
            ...
            else -> R.drawable.lemon_restart // else durumuna restart kalıyor
        }
